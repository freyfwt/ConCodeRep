...
class BatchProgramClassifier(nn.Module):
    ...
    def forward(self, x1, x2, x3):
        ...
        inputs = torch.cat([code, calling], dim=0)
...
if __name__ == '__main__':
    ...
    # training procedure
    # state_dict = torch.load(MODEL_DIR + '/class_max_pool_calling.pkl')
    ...
    model = best_model
    torch.save(model.state_dict(), MODEL_DIR + '/class_max_pool_calling.pth')
    ...
    model = BatchProgramClassifier(EMBEDDING_DIM, HIDDEN_DIM, MAX_TOKENS + 1, ENCODE_DIM, LABELS, BATCH_SIZE,
                                   USE_GPU, embeddings)
    model.load_state_dict(torch.load(MODEL_DIR + '/class_max_pool_calling.pth'))
