...
class BatchProgramClassifier(nn.Module):
    ...
    def forward(self, x1, x2, x3):
        code = self.encode(x1)
        calling = self.encode(x2)
        called = self.encode(x3)
        inputs = torch.cat([code, calling, called], dim=1)
...
def get_batch(dataset, idx, bs):
    ...
    for _, item in tmp.iterrows():
        x1.append(item['code'])
        x2.append(item['calling'])
        x3.append(item['called'])
        labels.append([item['label']])
    return x1, x2, x3, torch.FloatTensor(labels)
...
if __name__ == '__main__':
    ...
    train_data = pd.read_pickle(DATA_DIR + '/train_random_df.pkl').sample(frac=1, random_state=RANDOM_SEED)
    dev_data = pd.read_pickle(DATA_DIR + '/dev_random_df.pkl').sample(frac=1, random_state=RANDOM_SEED)
    test_data = pd.read_pickle(DATA_DIR + '/test_random_df.pkl').sample(frac=1, random_state=RANDOM_SEED)
    ...
    # training procedure
    # state_dict = torch.load(MODEL_DIR + '/class_concat_random.pth')
    ...
    for epoch in range(EPOCHS):
        ...
        while i < len(train_data):
            ...
            i += BATCH_SIZE
            train_code, train_calling, train_called, train_labels = batch
            if USE_GPU:
            ...
            output = model(train_code, train_calling, train_called)
        ...
        while i < len(dev_data):
            ...
            i += BATCH_SIZE
            dev_code, dev_calling, dev_called, dev_labels = batch
            ...
            model.hidden = model.init_hidden()
            output = model(dev_code, dev_calling, dev_called)
    ...
    model = best_model
    torch.save(model.state_dict(), MODEL_DIR + '/class_concat_random.pth')
    ...
    model = BatchProgramClassifier(EMBEDDING_DIM, HIDDEN_DIM, MAX_TOKENS + 1, ENCODE_DIM, LABELS, BATCH_SIZE,
                                   USE_GPU, embeddings)
    model.load_state_dict(torch.load(MODEL_DIR + '/class_concat_random.pth'))
    ...
    while i < len(test_data):
        ...
        test_code, test_calling, test_called, test_labels = batch
        if USE_GPU:
            test_labels = test_labels.cuda()

        model.batch_size = len(test_labels)
        model.hidden = model.init_hidden()
        output = model(test_code, test_calling, test_called)
